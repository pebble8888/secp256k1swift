...
fileprivate func CHECK(_ cond: Bool, _ line: Int = #line)
{
    assert(cond, "line \(line)")
}

fileprivate func VERIFY_CHECK(_ cond: Bool, _ line: Int = #line)
{
    assert(cond, "line \(line)")
}

fileprivate var g_count:Int = 64
fileprivate var ctx: secp256k1_context?
...
func test_rand_int(uint32_t range, uint32_t subrange) {
...
func run_field_convert() {
    ...
    CHECK(fes2.equal(fes)) //memcmp(&fes2, &fes, sizeof(fes)) == 0);
}
...
func run_field_misc() {
    ...
    for i in 0 ..< 5*g_count {
        ...
        CHECK(!zs.equal(zs)) // memcmp(&xs, &zs, sizeof(xs)) != 0);
...
func test_main(_ ch:String?) {
    ...
    //printf("test count = %i\n", count);
    //print("random seed = %02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x\n", seed16[0], seed16[1], seed16[2], seed16[3], seed16[4], seed16[5], seed16[6], seed16[7], seed16[8], seed16[9], seed16[10], seed16[11], seed16[12], seed16[13], seed16[14], seed16[15]);
